...
### This TuneReportCheckpointCallback is not updated by the official team, still using pytorch_lightning, while my lightning 2.0 uses pytorch.lightning
import ptwt, pywt
...
def main(Sample_config, args):
    ...
    print("Please confirm if the number of sensors is:", args.sensor_num, "Please confirm if the number of nodes (channels) is:", args.node_num)

    datamodule = MyLigDataModule(args)
    model = MyLigModel(args)
    # model = torch.compile(MyLigModel(args))
    # Cannot use for now, will cause error: https://github.com/Lightning-AI/pytorch-lightning/issues/18835
    # And I used in MyModel.py: self.MySTGNN = torch.compile(MySTGNN(self.args))
...
def light_main(Sample_config, args):
    ...
    # model = torch.compile(MyLigModel(args))
    # Cannot use for now, will cause error: https://github.com/Lightning-AI/pytorch-lightning/issues/18835
    # And I used in MyModel: self.MySTGNN = torch.compile(MySTGNN(self.args))
...
"""
Define ray tune hyperparameter optimization function
"""
...
def ray_tune_run(Search_config, args, everyexp_gpu, exp_num):
    ...
    """Define search algorithm, None means using default random search algorithm"""
    algo = None
    ...
    """num_workers=1 means don't split one experiment across multiple GPUs, don't change this"""
    ...
    if os.path.exists(log_path):
        print("Log for this experiment already exists, will continue training, but note that the search space will not change and remains the same as the previous experiment")
...
"""Configure search space"""
"""
# tune.choice means the search space is a list, randomly select one from it
# tune.loguniform means the search space is a log-uniform distribution, randomly select one from it
# tune.uniform means the search space is a uniform distribution, randomly select one from it
# tune.quniform means the search space is a uniform distribution, randomly select one from it, but according to given step size, like tune.quniform(0, 2, 0.2)
# tune.grid_search means the search space is a list, select one in order from it, ensuring specified values are sampled
"""



"## Experiment 892: 'Typical_Nonlinear_Operators1', G3CN+MutualInfo, no decomposition, reconstruct, V7.892"
Search_config = {
    ### Experiment settings
    'Version': 'V7.892',
    # 'Method': 'MadjGCN',
    ...
    # exp_name is set in main_sub.py
    ...
    # # This pred_step only needs to be set when forecast and pred_len is greater than lag, usually set to lag and 0.5*lag
    'num_workers': 3,
    'feature': 'M',
    ...
    'graph_ca_len': 3000,  # Set this larger because MIC has a large amount of data
    ...
    # spatial_block, [[[MAdjGCN is G3CN, CMTS_GCN is multi-layer G3CN]]]
    ...
    'self_edge': False,  # self-connection
    # If MAdjGCN is selected
    ...
    'residual_alpha': 0.0,  # inside spatial convolution block
    'block_residual': 0,  # outside spatial convolution block
    # 'self_edge': tune.choice([True, False]),    # self-connection
    # # If CMTS_GCN is selected, in addition to the above parameters, there are also the following parameters
    ...
    # # If GCN_s is selected
    # 'GCN_layer_nums': tune.choice([...]),
    # 'self_edge': tune.choice([True, False]),    # self-connection graph
    # 'block_residual': tune.choice([0, 1]),  # outside spatial convolution block
    # # If Muti_S_GAT is selected
    # 'S_GAT_K': tune.choice([...]),
    # 'S_GAT_embed_dim': tune.choice([...]),
    # 'use_gatv2': tune.choice([...]),
    # 'block_residual': tune.choice([...]),  # outside spatial convolution block
    # # If GPRGNN is selected
    # 'GPRGNN_K': tune.choice([...]),
    # # If H2GCN is selected
    ...
    # 'random_seed': 42,
    # 'max_epoch': 500,
    'patience': 5, # This must be set, stop training if loss doesn't decrease for 5 consecutive epochs

    ### Ray configuration
    'trail_grace_period': 20,    # Minimum number of epochs before early stopping a trail
    'trail_reduction_factor': 3,
    'grid_num_samples': 3,  # This must be set, how many parameter groups to sample from search_space
}
devices = "0,1,2,3"
